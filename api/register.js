...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (path === 'generate_register_token' && method === 'POST') {
      ...
      const enc = CryptoJS.AES.encrypt(JSON.stringify(tokenData), ADMIN_KEY).toString();
      await db.ref('register_tokens/' + token).set({ data: enc });
      
      return res.status(200).json(encryptResponse({ success: true, token: token, expiry: expiry }));
    }
    ...
    if (path === 'register' && method === 'POST') {
      ...
      if (!registerToken) {
        return res.status(200).json(encryptResponse({ success: false, error: 'token_required', message: 'Sesi pendaftaran tidak valid! Refresh halaman.' }));
      }

      const tokenSnap = await db.ref('register_tokens/' + registerToken).once('value');
      const tokenRaw = tokenSnap.val();
      if (!tokenRaw?.data) {
        return res.status(200).json(encryptResponse({ success: false, error: 'token_invalid', message: 'Sesi pendaftaran tidak valid! Refresh halaman.' }));
      }

      const tokenData = await decryptData(tokenRaw);
      if (!tokenData?.token) {
        return res.status(200).json(encryptResponse({ success: false, error: 'token_invalid', message: 'Sesi pendaftaran tidak valid! Refresh halaman.' }));
      }

      if (tokenData.used === true) {
        return res.status(200).json(encryptResponse({ success: false, error: 'token_used', message: 'Sesi pendaftaran sudah digunakan! Refresh halaman.' }));
      }

      if (tokenData.expiry && Date.now() > tokenData.expiry) {
        return res.status(200).json(encryptResponse({ success: false, error: 'token_expired', message: 'Sesi pendaftaran sudah expired! Refresh halaman.' }));
      }
      ...
      const updatedTokenData = { ...tokenData, used: true, usedAt: Date.now(), usedBy: username };
      const tokenEnc = CryptoJS.AES.encrypt(JSON.stringify(updatedTokenData), ADMIN_KEY).toString();
      await db.ref('register_tokens/' + registerToken).update({ data: tokenEnc });
