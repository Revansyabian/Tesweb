...
async function deleteAllHistory() {
    ...
    try {
        ...
        if (!transactions || typeof transactions !== 'object' || Object.keys(transactions).length === 0) { 
            hideLoading(); Swal.fire({ icon: "warning", title: "Oops...", text: "Tidak ada riwayat!", confirmButtonColor: "#0ea5e9" }); return; 
        }
        var keys = Object.keys(transactions); var count = 0;
        for (var i = 0; i < keys.length; i++) { 
            try { await callRevanstore('transactions/' + keys[i], 'DELETE'); count++; } catch(e) {}
            await new Promise(function(r) { setTimeout(r, 1000); });
        }
        hideLoading();
        Swal.fire({ icon: "success", title: "Berhasil!", text: count + " riwayat dihapus!", timer: 2000, showConfirmButton: false });
...
async function login() {
    ...
    try {
        ...
        if (result && result.success) {
            ...
            if (expiryCheck.expired) { hideLoading(); showExpiredBanner(); loginInProgress = false; return; }

            currentUser = { 
                id: user.id, username: user.username, password: password, 
                role: user.role || 'Operator', full_name: user.full_name || user.username, 
