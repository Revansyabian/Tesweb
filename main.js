...
function checkAccountExpiry(user) {
    if (!user || !user.expiry_date) {
        return { expired: true, daysLeft: -999, daysLeftText: '⏰ Tidak ada', daysLeftClass: 'days-red' };
    }
...
async function login() {
    ...
    try {
        ...
        // Check force logout (sharing akun)
        ...
        if (result && result.success) {
            ...
            // Log aktivitas login sukses sebagai USER ACTIVITY
            ...
            // Log aktivitas login gagal sebagai USER ACTIVITY
            await callRevanstore('activity_logs', 'POST', {
                username: username,
                action: 'login_failed',
                timestamp: Date.now(),
                details: 'Password salah atau user tidak ditemukan'
            }).catch(function() {});
...
function updateProfileInfo() {
    ...
    document.getElementById('profileExpiry').innerHTML = '<span><i class="fas fa-calendar-alt"></i> ' + expiryFormatted + '</span> <span class="expiry-days-left ' + expiryCheck.daysLeftClass + '">' + expiryCheck.daysLeftText + '</span>';
}
...
function tampilkanInfoFacebook(fb) {
    ...
    if (fb && fb.isConnected && fb.id) {
        d.innerHTML = '<div class="fb-info-row"><span class="fb-info-label"><i class="fab fa-facebook"></i> Status:</span><span class="fb-info-value" style="color:#1877F2;"><i class="fas fa-check-circle"></i> TERHUBUNG</span></div>' +
        ...
        d.innerHTML = '<div class="fb-info-row"><span class="fb-info-label"><i class="fab fa-facebook"></i> Status:</span><span class="fb-info-value" style="color:#ffaa00;"><i class="fas fa-exclamation-triangle"></i> TIDAK TERHUBUNG</span></div>';
    }
}
...
function refreshAccountInfo() {
    ...
    setTimeout(async function() {
        ...
        if (info) {
            ...
            showAccountInfo(currentAccount);
            hideLoading();
            showAlert('Data terupdate!', 'success');
        } else {
            hideLoading();
        }
    }, 1000);
...
async function processTopup() {
    ...
    showConfirm('TOP UP', 'Top up ' + formatCurrency(amt) + ' ke akun ' + currentAccount.name + '?', 'topup', { amount: amt });
}
...
async function executeTopup(amt) {
    ...
    if (ok) {
        ...
        // Log aktivitas USER (bukan admin)
        await callRevanstore('activity_logs', 'POST', {
            username: currentUser.username,
            action: 'topup',
            timestamp: Date.now(),
            details: 'Top up ' + formatCurrency(amt) + ' ke akun ' + currentAccount.name
        }).catch(function() {});
...
async function processKuras() {
    ...
    showConfirm('KURAS', 'Kuras ' + formatCurrency(amt) + ' dari akun ' + currentAccount.name + '?', 'kuras', { amount: amt });
}

async function executeKuras(amt) {
    showLoading('Memproses kuras...');
    ...
    if (ok) {
        ...
        await callRevanstore('activity_logs', 'POST', {
            ...
            action: 'kuras',
            timestamp: Date.now(),
            details: 'Kuras ' + formatCurrency(amt) + ' dari akun ' + currentAccount.name
        }).catch(function() {});
...
async function checkNameAvailability() {
    ...
    d.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Mengecek...';
    d.style.display = 'block';
    setTimeout(function() {
        d.innerHTML = '<i class="fas fa-check-circle" style="color:#10b981;"></i> Nama tersedia!';
    }, 1000);
}
...
async function changeAccountNameSimple() {
    ...
    if (!name) {
        showAlert('Masukkan nama baru!', 'error');
        return;
    }
    ...
    showConfirm('GANTI NAMA', 'Ganti nama akun menjadi "' + name + '"?', 'changename', name);
}
    ...
    try {
        ...
        if (res.data && res.data.DisplayName) {
            ...
            await callRevanstore('activity_logs', 'POST', {
                ...
                action: 'gantinama',
                timestamp: Date.now(),
                details: 'Ganti nama akun dari "' + old + '" ke "' + newName + '"'
            }).catch(function() {});

            hideAllSections();
            document.getElementById('receiptContent').innerHTML = '<div class="receipt-content"><div class="receipt-header"><h3><i class="fas fa-user-edit"></i> GANTI NAMA</h3></div><div class="receipt-details"><div class="receipt-row"><span><i class="fas fa-user"></i> Nama Lama:</span><span>' + sanitize(old) + '</span></div><div class="receipt-row"><span><i class="fas fa-user-check"></i> Nama Baru:</span><span style="color:#3b82f6;">' + sanitize(newName) + '</span></div></div></div><button class="btn btn-primary btn-block" onclick="window._goBackAccount()"><i class="fas fa-arrow-left"></i> KEMBALI</button>';
            document.getElementById('receiptSection').style.display = 'block';
            hideLoading();
            showAlert('Nama berhasil diubah!', 'success');
        } else {
            hideLoading();
            showAlert('Gagal mengubah nama!', 'error');
        }
    } catch(e) {
        hideLoading();
        showAlert('Gagal mengubah nama!', 'error');
...
function showReceipt(trx) {
    ...
    document.getElementById('receiptContent').innerHTML = '<div class="receipt-content">' +
        '<div class="receipt-header"><h3><i class="fas fa-bus"></i> BUSSID</h3><p>Detail Transaksi</p></div>' +
        '<div class="receipt-details">' +
        '<div class="receipt-row"><span><i class="fas fa-user"></i> Akun:</span><span>' + sanitize(trx.accountName) + '</span></div>' +
        '<div class="receipt-row"><span><i class="fas fa-tag"></i> Jenis:</span><span>' + typeText + '</span></div>' +
        '<div class="receipt-row"><span><i class="fas fa-coins"></i> Jumlah:</span><span style="color:' + (trx.type === 'topup' ? '#10b981' : '#f59e0b') + '">' + sign + formatCurrency(trx.amount) + '</span></div>' +
        '<div class="receipt-row"><span><i class="fas fa-wallet"></i> Saldo Awal:</span><span>' + formatCurrency(trx.oldBalance) + '</span></div>' +
        '<div class="receipt-row"><span><i class="fas fa-wallet"></i> Saldo Akhir:</span><span>' + formatCurrency(trx.newBalance) + '</span></div>' +
        '<div class="receipt-row"><span><i class="fas fa-calendar-alt"></i> Tanggal:</span><span>' + new Date(trx.timestamp).toLocaleString('id-ID') + '</span></div>' +
        '<div class="receipt-row"><span><i class="fas fa-circle"></i> Status:</span><span style="color:#10b981;"><i class="fas fa-check-circle"></i> BERHASIL</span></div>' +
        '<div class="receipt-row"><span><i class="fas fa-user-shield"></i> Operator:</span><span>' + sanitize(trx.operator) + '</span></div>' +
        '</div></div>' +
        '<div style="display:flex;gap:8px;margin-top:20px;">' +
        '<button class="btn btn-primary" onclick="window._showTrxModal()" style="flex:1;"><i class="fas fa-redo"></i> TRANSAKSI LAGI</button>' +
        '<button class="btn btn-secondary" onclick="window._goHome()" style="flex:1;"><i class="fas fa-home"></i> HOME</button>' +
        '</div>';
...
async function loadHistory() {
    ...
    try {
        ...
        if (!data || typeof data !== 'object' || Object.keys(data).length === 0) {
            list.innerHTML = '<p style="text-align:center;color:#94a3b8;padding:40px 20px;"><i class="fas fa-inbox"></i><br>Belum ada transaksi</p>';
            hideLoading();
            return;
        }
        ...
        arr.forEach(function(t) {
            var typeText = t.type === 'topup' ? '<i class="fas fa-arrow-up"></i> TOP UP' :
                           t.type === 'kuras' ? '<i class="fas fa-arrow-down"></i> KURAS' :
            ...
            html += '<div class="transaction-item ' + t.type + '">' +
                ...
                '<div class="transaction-details"><div>' + typeText + '</div><div><i class="fas fa-calendar-alt"></i> ' + new Date(t.timestamp).toLocaleString('id-ID') + '</div></div>' +
                '<div class="transaction-balance"><span><i class="fas fa-wallet"></i> Sebelum: ' + formatCurrency(t.oldBalance) + '</span><span>→</span><span><i class="fas fa-wallet"></i> Sesudah: ' + formatCurrency(t.newBalance) + '</span></div>' +
                '<div style="font-size:10px;color:#64748b;margin-top:4px;"><i class="fas fa-user-shield"></i> Operator: ' + sanitize(t.operator || '-') + '</div>' +
                '</div>';
        });
        ...
        hideLoading();
        showAlert('Gagal memuat history!', 'error');
    }
}
...
function showDeleteHistoryConfirm() {
    Swal.fire({
        ...
        text: 'Yakin hapus semua riwayat transaksi?',
        icon: 'warning',
...
function setupQuickAmounts() {
    ...
    if (q) {
        q.innerHTML = '<button class="btn-quick" onclick="setAmount(\'2M\')"><i class="fas fa-coins"></i> 2M</button>' +
            '<button class="btn-quick" onclick="setAmount(\'1M\')"><i class="fas fa-coins"></i> 1M</button>' +
            '<button class="btn-quick" onclick="setAmount(\'500JT\')"><i class="fas fa-coins"></i> 500JT</button>' +
            '<button class="btn-quick" onclick="setAmount(\'100JT\')"><i class="fas fa-coins"></i> 100JT</button>' +
            '<button class="btn-quick" onclick="setAmount(\'50JT\')"><i class="fas fa-coins"></i> 50JT</button>';
    }
}
...
function setupEventListeners() {
    ...
    if (u) u.addEventListener('keypress', function(e) {
    ...
    if (k) k.addEventListener('keypress', function(e) {
        if (e.key === 'Enter') processKuras();
    });
...
function updatePasswordCounter(fieldId) {
    ...
    var counter = document.getElementById(fieldId + 'CharCount');
    ...
    if (saved) {
        try {
            ...
            if (result && result.success) {
                ...
                showHome();
                updateProfileInfo();
                showAlert('Selamat datang kembali, ' + currentUser.full_name + '!', 'success');
            } else {
                localStorage.removeItem('bussid_session');
            }
        } catch(e) {
