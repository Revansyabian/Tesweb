...
function showAlert(message, type, duration) {
    ...
    alertDiv.innerHTML = '<div class="alert-content"><i class="fas ' + (icons[type] || 'fa-info-circle') + '"></i><span>' + message + '</span></div>';
...
function hideAllSections() {
    var sections = ['accountInfo', 'topupSection', 'kurasSection', 'historySection', 'settingsSection', 'receiptSection'];
...
function updateProfileInfo() {
    ...
    if (currentUser.last_login) { var lastLoginDate = new Date(currentUser.last_login); document.getElementById('profileLastLogin').textContent = lastLoginDate.toLocaleString('id-ID'); }
    else { document.getElementById('profileLastLogin').textContent = 'Belum pernah login'; }
}
...
async function loginWithDeviceId(deviceId) {
    ...
    try {
        ...
        if (cleanInput.includes('.')) {
            ...
            else throw new Error('Token tidak valid!');
        } else {
            ...
            if (data.data && data.data.SessionTicket) {
                ...
                var userInfo = await getUserInfoFromPlayFab();
                if (userInfo) { currentAccount = { deviceId: cleanDeviceId, name: userInfo.name, balance: userInfo.balance, facebook: userInfo.facebook, facebookAvatarUrl: userInfo.facebookAvatarUrl, playFabId: userInfo.playFabId }; return true; }
            }
        }
...
function tampilkanInfoFacebook(facebookData) {
    ...
    if (facebookData && facebookData.isConnected && facebookData.id) {
        fbDetails.innerHTML = '<div class="fb-info-row"><span class="fb-info-label"><i class="fab fa-facebook"></i> Status:</span><span class="fb-info-value" style="color:#1877F2;">✅ TERHUBUNG</span></div>' +
            '<div class="fb-info-row"><span class="fb-info-label"><i class="fab fa-facebook"></i> Facebook ID:</span><span class="fb-info-value" style="font-family:monospace;font-size:12px;">' + facebookData.id + '</span></div>' +
            '<div class="fb-info-row"><span class="fb-info-label"><i class="fas fa-user"></i> Nama Facebook:</span><span class="fb-info-value">' + (facebookData.name || '-') + '</span></div>' +
            '<div class="fb-info-row"><span class="fb-info-label"><i class="fas fa-envelope"></i> Email:</span><span class="fb-info-value">' + (facebookData.email || '-') + '</span></div>';
    } else {
...
function showAccountInfo(accountInfo) {
    document.getElementById('accountName').textContent = accountInfo.name || 'Tidak diketahui';
...
function showReceipt(transaction) {
    ...
    document.getElementById('receiptContent').innerHTML =
        ...
        '<div class="receipt-row"><span>Nama Akun:</span><span><strong>' + transaction.accountName + '</strong></span></div>' +
        ...
        '<div style="display:flex;gap:10px;margin-top:20px;"><button class="btn btn-success" onclick="showTrxLagiModal()" style="flex:1;"><i class="fas fa-sync-alt"></i> TRX LAGI</button><button class="btn btn-primary" onclick="showHome()" style="flex:1;"><i class="fas fa-home"></i> HOME</button></div>';
...
function showConfirm(title, message, action, data) { document.getElementById('confirmTitle').innerHTML = '<i class="fas fa-exclamation-triangle"></i> ' + title; document.getElementById('confirmMessage').innerHTML = message; pendingAction = action; pendingData = data; document.getElementById('confirmModal').classList.add('active'); }
function cancelConfirm() { pendingAction = null; pendingData = null; document.getElementById('confirmModal').classList.remove('active'); }
async function confirmAction() { if (!pendingAction || !pendingData) return; document.getElementById('confirmModal').classList.remove('active'); if (pendingAction === 'topup') await executeTopup(pendingData.amount); else if (pendingAction === 'kuras') await executeKuras(pendingData.amount); pendingAction = null; pendingData = null; }
...
async function changeAccountNameSimple() {
    ...
    try {
        ...
        if (result.data && result.data.DisplayName) {
            ...
            showNameChangeModal('Berhasil ganti dari "' + oldName + '" ke "' + newName + '"', 'success');
            document.getElementById('newAccountName').value = '';
            document.getElementById('nameAvailability').style.display = 'none';
        }
    } catch (error) { showNameChangeModal('Gagal mengubah nama!', 'error'); }
}
...
function showNameChangeModal(message, type) {
    ...
    var icons = { success: 'fa-check-circle', error: 'fa-exclamation-circle', warning: 'fa-exclamation-triangle', info: 'fa-info-circle' };
    msg.innerHTML = '<i class="fas ' + icons[type] + '"></i> ' + message;
